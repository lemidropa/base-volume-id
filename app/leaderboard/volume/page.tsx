...
async function getTop(page: number, pageSize: number) {
  const url = `${process.env.NEXT_PUBLIC_BASE_URL || ''}/api/leaderboard/volume?page=${page}&pageSize=${pageSize}`;
  const res = await fetch(url, { cache: 'no-store' });
  if (!res.ok) return { rows: [], total: 0 } as any;
  return res.json();
}

export default async function VolumeLeaderboardPage({ searchParams }: { searchParams?: { page?: string; pageSize?: string } }) {
  ...
  return (
    <main className="max-w-4xl mx-auto p-8">
      <h1 className="text-3xl font-bold mb-6">Top Volume IDs</h1>
          ...
          <tbody>
            {rows.map((r: any, i: number) => (
              <tr key={r.userId} className="border-b border-slate-900">
                ...
                <td className="py-2 pr-4">{r.username || r.userId.slice(0, 6) + '…'}</td>
